...
export function buildResumeRecoverySummary(sessionId: string, config: Config): string {
  const transcriptPath = findClaudeTranscript(sessionId, config.claudeWorkDir);
  if (!transcriptPath) {
    return [
      `Previous Claude session ${sessionId} could not be resumed.`,
      'No local Claude transcript file for that session was found on this machine.',
...
function findClaudeTranscript(sessionId: string, workDir: string): string | undefined {
  const projectsDir = path.join(os.homedir(), '.claude', 'projects');
  const directCandidates = [
    path.join(projectsDir, projectKey(workDir), `${sessionId}.jsonl`),
    path.join(projectsDir, projectKey(process.cwd()), `${sessionId}.jsonl`),
  ];
  ...
  return findFileByName(projectsDir, `${sessionId}.jsonl`);
}
